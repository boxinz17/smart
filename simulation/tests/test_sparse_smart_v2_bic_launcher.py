...
spec = importlib.util.spec_from_file_location("bic_launcher", HPC / "submit_sparse_smart_v2_bic.py")
...
def test_dry_run_declares_independent_pools_and_separate_summary(campaign, monkeypatch):
    root, plan = campaign
    ...
    assert all("--dependency=afterok:PREPARE_JOB_ID" in row["command"] for row in pools)
...
def test_changed_source_stops_before_submission(campaign, monkeypatch):
    root, plan = campaign
    calls = scheduler(monkeypatch)
    path = root / "source/hpc/discovery/sparse_smart_v2_bic_worker.sh"
...
def test_scripts_parse_and_preserve_one_cpu_srun_dispatch():
    for suffix in ("prepare.sbatch", "pool.sbatch", "summary.sbatch", "worker.sh"):
        script = HPC / ("sparse_smart_v2_bic_" + suffix)
        subprocess.run(["bash", "-n", str(script)], check=True, capture_output=True)
    worker = (HPC / "sparse_smart_v2_bic_worker.sh").read_text()
    assert "srun --exclusive --exact --nodes=1 --ntasks=1 --cpus-per-task=1" in worker
    assert "run_sparse_smart_v2_bic.py" in worker and "'%06d'" in worker
    pool = (HPC / "sparse_smart_v2_bic_pool.sbatch").read_text()

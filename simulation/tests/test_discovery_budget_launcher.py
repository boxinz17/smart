...
def test_full_dry_run_has_exact_headerless_grid_and_distributed_resources(checkout, tmp_path):
    result, roots, archive_root = dry_run(checkout, tmp_path, ("--workers", "32"))
    ...
    assert len({row[0] for row in rows}) == 7200
    assert all(len(row) == 5 and row[0].startswith("m") for row in rows)
    assert rows[0] == ["m0_e0_s0_k0", "0", "0", "0", "0"]
    assert plan["expected_applicable"] == 6300
    assert plan["expected_inapplicable"] == 900
    assert plan["configuration"]["iteration_budgets"] == [500, 1000, 2000, 4000, 8000]
    assert plan["configuration"]["penalties_u"] == plan["configuration"]["penalties_v"] == [.0025, .01, .04]
...
def worker_run(tmp_path):
    run, archive, bin_dir = tmp_path / "run", tmp_path / "archive", tmp_path / "bin"
    source = run / "source"
...
def test_pool_still_aggregates_and_summarizes_after_worker_failure(worker_run, tmp_path):
...
""", python=True)
    ...
    assert any(call[0].endswith("summarize_sparse_smart_budget_study.py") for call in calls)
    status = (run / "stage-status.tsv").read_text()
    assert "workers\t3" in status and "aggregate\t4" in status and "summary\t0" in status

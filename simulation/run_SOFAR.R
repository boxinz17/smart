...
run_simulation <- function(n, r, suffix) {
  ...
  res <- sofar(Y, X, nrank = r, ic.type = "BIC")
  C_hat <- if (r == 1) res$U %*% res$D %*% t(res$V) else res$U %*% diag(res$D) %*% t(res$V)
  ...
  out_file <- sprintf("%s/SOFAR_result_%s_%s_%s_rd_seed_id=%d.json", out_dir, model, exp, suffix, rd_seed_id)

...
run_simulation <- function(n, r, suffix) {
  ...
  X <- data$X; Y <- data$Y; C_true <- data$C_star

  res <- rssvd(Y, X, nrank = r, ic.type = "BIC")
  C_hat <- if (r == 1) res$U %*% t(res$V) * res$D else res$U %*% diag(res$D) %*% t(res$V)
  err <- evaluate_model_avg_err(C_hat, C_true)

  cat(sprintf("\n=== Average Frobenius Norm Error ===\nRSSVD: %.4f\n", err))

  result <- list(
    C_true = unname(split(as.matrix(C_true), row(C_true))),
    C_hat = unname(split(as.matrix(C_hat), row(C_hat))),
    avg_err = err
  )

  out_dir <- sprintf("result/%s/%s", model, exp)
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  out_file <- sprintf("%s/RSSVD_result_%s_%s_%s_rd_seed_id=%d.json", out_dir, model, exp, suffix, rd_seed_id)

#!/usr/bin/env python3
"""Submit frozen comparator groups to serial GNU Parallel pools, without arrays.
...
def load_inputs(root):
    ...
    require(plan.get("schema_version") == 1 and plan.get("method") == "PaperSourceComparators", "Unsupported comparator plan")
    ...
    for name in ("paper_source_comparators_prepare.sbatch", "paper_source_comparators_pool.sbatch",
                 "paper_source_comparators_worker.sh", "paper_source_comparators_summary.sbatch"):
...
def ready(root, plan):
    ...
    return canary.exists() and read(canary).get("success") is True and read(canary).get("plan_fingerprint") == plan["plan_fingerprint"]
...
def command(root, args, tag, kind):
    ...
    cmd = ["sbatch", "--parsable", "--job-name=pscomp-" + hashlib.sha256(str(root).encode()).hexdigest()[:8] + "-" + tag,
...
def summary_submission(root, args, identity, pools):
    ...
    cmd += [str(root / "source/hpc/discovery/paper_source_comparators_summary.sbatch"), str(root), tag]
...
def orchestrate(root, args):
    ...
    if ready(root, plan):
        ...
        if state in ("active", "not_queried"):
            ...
            cmd += [str(root / "source/hpc/discovery/paper_source_comparators_prepare.sbatch"), str(root), tag]
    ...
    if not args.prepare_only:
        for index in sorted(selected):
            ...
            if pools:
                cmd.append("--dependency=afterany:" + pools[-1])
            elif dependency:
                cmd.append("--dependency=afterok:" + dependency)
            cmd += [str(root / "source/hpc/discovery/paper_source_comparators_pool.sbatch"), str(root), str(args.workers), name, tag]
...
def main(argv=None):
    ...
    parser.add_argument("--max-tasks-per-chunk", type=int, default=5000)
    ...
    parser.add_argument("--summary-time", default="24:00:00")

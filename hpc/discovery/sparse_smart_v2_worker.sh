...
run_dir=$(cd -- "$2" && pwd -P)
task_id=$3
printf -v task_name '%05d' "$task_id"
...
if [[ "$mode" == dispatch ]]; then
    ...
    srun --exclusive --exact --nodes=1 --ntasks=1 --cpus-per-task=1 --kill-on-bad-exit=0 \
        bash "$run_dir/source/hpc/discovery/sparse_smart_v2_worker.sh" execute "$run_dir" "$task_id" </dev/null
    exit 0
fi
...
cd "$task_dir"
command_args=("$VENV/bin/python" "$SMART_SOURCE_ROOT/simulation/run_sparse_smart_v2_pilot.py" fit --root "$run_dir" --task "$task_id")
{
    printf 'Job: %s\nStep: %s\nHost: %s\nTask: %s\nParallel sequence: %s\n' \

...
if [[ "$mode" == dispatch ]]; then
    ...
    srun --exclusive --exact --nodes=1 --ntasks=1 --cpus-per-task=1 --kill-on-bad-exit=0 \
        bash "$run_dir/source/hpc/discovery/paper_source_comparators_worker.sh" execute "$run_dir" "$task_id" </dev/null
    exit 0
fi
...
cd "$task_dir"
command_args=("$VENV/bin/python" -m paper_source_comparators_20260913.runtime fit --root "$run_dir" --task "$task_id")
{
    printf 'Job: %s\nStep: %s\nHost: %s\nTask: %s\nParallel sequence: %s\n' \
